    ...
    <group position={building.position}>
      ...
      {building.windows && (
        <>
          {/* Front windows */}
          <Box
            position={[0, 0, building.size[2] / 2 + 0.01]}
            scale={[building.size[0] * 0.8, building.size[1] * 0.8, 0.1]}
          >
      ...
      {building.neonSign && (
        <Box
          position={[0, building.size[1] / 2 + 1, building.size[2] / 2 + 0.1]}
          scale={[building.size[0] * 0.6, 1, 0.1]}
        >
          <meshBasicMaterial
            color="#ff0066"
            emissive="#ff0066"
...
const Player: React.FC<{
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === ' ') {
        const now = Date.now();
        if (now - lastDashTime > 1000 && stamina > 20) {
          // 1 second cooldown
          setLastDashTime(now);
  ...
  useFrame((state, delta) => {
    ...
    if (!checkCollision(testPosition, buildings)) {
      ...
      const testX: [number, number, number] = [
        newPosition[0] + newVelocity[0] * delta,
        newPosition[1],
        newPosition[2],
      ];
      const testZ: [number, number, number] = [
        newPosition[0],
        newPosition[1],
        newPosition[2] + newVelocity[2] * delta,
      ];
    ...
    camera.position.x +=
      (targetCameraX - camera.position.x) * cameraSmoothing * delta;
    camera.position.y +=
      (newPosition[1] + cameraHeight - camera.position.y) *
      cameraSmoothing *
      delta;
    camera.position.z +=
      (targetCameraZ - camera.position.z) * cameraSmoothing * delta;
        ...
        <group>
          ...
          <Box position={[0, -0.8, 0]} scale={[0.5, 0.1, 0.5]}>
            <meshBasicMaterial
              ...
              opacity={Math.min(
                0.3,
                (Math.abs(velocity[0]) + Math.abs(velocity[2])) * 0.1
              )}
            />
          </Box>
        </group>
...
const Game: React.FC = () => {
  ...
  const [explosions, setExplosions] = useState<
    Array<{ id: string; position: [number, number, number]; intensity: number }>
  >([]);
  const [bloodPools, setBloodPools] = useState<
    Array<{ id: string; position: [number, number, number]; size: number }>
  >([]);
  ...
  const handlePlayerEnterVehicle = useCallback(
    (vehicleId: string) => {
  ...
  const handlePoliceKilled = useCallback(
    (policeId: string) => {
  ...
  const handleTimeUpdate = useCallback(
    (currentTime: number, isNight: boolean) => {
    ...
    <div className="w-full h-screen bg-black relative overflow-hidden">
      ...
      >
        ...
        {allBuildings.map((building, index) => {
          const distanceSq =
            (building.position[0] - playerPosition[0]) *
              (building.position[0] - playerPosition[0]) +
            (building.position[2] - playerPosition[2]) *
              (building.position[2] - playerPosition[2]);
        ...
        {worldChunks.map((chunk) =>
          chunk.props
            ...
            .map((prop) => {
              const distanceSq =
                (prop.position[0] - playerPosition[0]) *
                  (prop.position[0] - playerPosition[0]) +
                (prop.position[2] - playerPosition[2]) *
                  (prop.position[2] - playerPosition[2]);
      ...
      <div className="absolute top-4 left-4 p-4 bg-black/90 text-white rounded-lg border border-red-500/70 backdrop-blur-sm">
        ...
        <div className="space-y-2 text-sm">
          <div className="flex justify-between">
            <span>Health:</span>
            <span
              className={`font-bold ${gameStore.playerStats.health < 30 ? 'text-red-400 animate-pulse' : 'text-green-400'}`}
            >
          ...
          <div className="flex justify-between">
            ...
            <span
              className={`font-bold ${threatLevel > 0.7 ? 'text-red-400' : threatLevel > 0.4 ? 'text-yellow-400' : 'text-green-400'}`}
            >
      ...
      {selectedNPC && (
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 p-4 bg-black/95 text-white rounded-lg border border-red-500/70 backdrop-blur-sm max-w-md z-40">
          <h3 className="font-bold text-red-400 mb-2 capitalize">
            {selectedNPC.type}
          </h3>
      ...
      <div className="absolute bottom-4 right-4 p-3 glass-panel text-white rounded text-xs border border-gray-600/50">
        ...
        <div>
          FPS:{' '}
          <span
            className={`font-bold ${frameRate > 45 ? 'text-green-400' : frameRate > 30 ? 'text-yellow-400' : 'text-red-400'}`}
          >
            {frameRate}
          </span>
        </div>
        <div>
          ...
          <span className="text-blue-400">
            {allNPCs.filter((npc) => !npc.isDead).length}
          </span>
        </div>
        <div>
          Effects:{' '}
          <span className="text-purple-400">
            {combatEffects.length + explosions.length}
          </span>
        </div>
        <div>
          Kills:{' '}
          <span className="text-red-400">
            {killStreak > 0 ? `${killStreak}x STREAK` : '0'}
          </span>
        </div>
      </div>
      ...
      <div className="absolute bottom-44 left-4 p-3 glass-panel text-white rounded text-xs border border-gray-600/50 max-w-xs">
        ...
        <div className="grid grid-cols-2 gap-1 text-xs">
          ...
          <span>Mouse:</span>
          <span className="text-gray-300">Look/Attack</span>
          <span>F:</span>
          <span className="text-gray-300">Enter Vehicle</span>
          <span>1-6:</span>
      ...
      {(screenEffects.shake > 0 ||
        ...
        <div
          ...
          style={{
            ...
            animation:
              screenEffects.shake > 0
                ? 'screen-shake 0.1s ease-in-out 5'
                : undefined,
            boxShadow:
              screenEffects.flash > 0
                ? `inset 0 0 100px rgba(255, 255, 255, ${screenEffects.flash})`
                : undefined,
          }}
        />
      )}
