    ...
    <>
      ...
      <div className="absolute top-4 left-4 p-4 glass-panel text-white rounded-lg border border-red-500/70 min-w-[320px]">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold neon-text animate-glow">
            🏴‍☠️ CRIMINAL STATUS
          </h2>
          <button
        ...
        <div className="space-y-4">
          ...
          <div
            className={`space-y-2 p-2 rounded ${gameStore.playerStats.health < 30 ? 'animate-danger-pulse' : ''}`}
          >
            <div className="flex justify-between items-center">
              <span className="flex items-center text-sm">
                <Heart
                  className={`h-4 w-4 mr-2 ${gameStore.playerStats.health < 30 ? 'animate-pulse text-red-400' : 'text-red-500'}`}
                />
                Health
              </span>
            ...
            <div className="w-full bg-gray-800 rounded-full h-3 overflow-hidden">
              <div
                className={`h-3 rounded-full transition-all duration-500 ${
                  gameStore.playerStats.health > 70
                    ? 'bg-green-500'
                    : gameStore.playerStats.health > 30
                      ? 'bg-yellow-500'
          ...
          {/* Wanted Level */}
          <div
            className={`flex justify-between items-center p-2 rounded ${gameStore.playerStats.wanted > 2 ? 'animate-danger-pulse' : ''}`}
          >
            <span className="flex items-center text-sm">
              <AlertTriangle
                className={`h-4 w-4 mr-2 ${gameStore.playerStats.wanted > 2 ? 'animate-pulse text-red-400' : 'text-yellow-500'}`}
              />
              Heat Level
            </span>
          ...
          <div className="space-y-2">
            ...
            <div className="w-full bg-gray-800 rounded-full h-2 overflow-hidden">
              <div
                ...
                style={{
                  width: `${Math.min(gameStore.playerStats.reputation, 100)}%`,
                }}
              >
                <div className="h-full bg-gradient-to-r from-transparent to-white opacity-30"></div>
              </div>
      ...
      {showSettings && (
        <div className="absolute top-4 left-4 ml-[340px] p-4 glass-panel text-white rounded-lg border border-red-500/70 animate-slide-in">
          <h3 className="text-lg font-bold text-red-400 mb-4 neon-text">
            SETTINGS
          </h3>
      ...
      <div className="absolute bottom-4 right-4 p-3 glass-panel text-white rounded text-xs border border-gray-600/50">
        ...
        <div>
          ...
          <span className="text-red-400">
            {gameStore.recentActions.filter((a) => a.includes('killed')).length}
          </span>
        </div>
        <div>
