...
const SmartNPC: React.FC<SmartNPCProps> = ({
  ...
  const makeDecision = () => {
    ...
    const threatLevel =
      (playerWanted * 2 + playerRep / 10 + playerKills / 5) / 15;
        ...
        if (state.target) {
          ...
          if (distanceToTarget < 5 && Math.random() > 0.93) {
            ...
            gameStore.updateStats({
              health: Math.max(0, gameStore.playerStats.health - damage),
            });
  ...
  const handleClick = () => {
    ...
    if (state.health <= 0) {
      ...
      gameStore.updateStats({
        reputation: gameStore.playerStats.reputation + repGain,
      });
    }
  };
  ...
  return (
    <group ref={meshRef} position={position}>
      <SpriteCharacter
        ...
        bloodLevel={
          state.mood === 'dead' ? 1 : Math.max(0, 1 - state.health / 100)
        }
