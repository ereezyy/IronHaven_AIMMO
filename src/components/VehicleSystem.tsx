...
const VehicleSystem: React.FC<VehicleSystemProps> = ({
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const types: DrivableVehicle['type'][] = [
        'sedan',
        'sports',
        'truck',
        'police',
    ...
    setVehicles((prev) =>
      prev.map((vehicle) => {
        if (vehicle.id === playerVehicle && vehicle.isPlayerDriving) {
          ...
          // Steering
          const speedSq = newVelocity[0] * newVelocity[0] + newVelocity[2] * newVelocity[2];
          if (speedSq > 1) { // 1^2 is 1
        ...
        >
          ...
          <Box position={[0, 0, -1.2]} scale={[2, 0.3, 0.1]}>
            <meshStandardMaterial
              ...
              emissive="#ff0000"
              emissiveIntensity={
                keys['s'] && vehicle.isPlayerDriving ? 0.8 : 0.1
              }
            />
          </Box>
