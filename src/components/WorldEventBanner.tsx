...
const WorldEventBanner: React.FC<WorldEventBannerProps> = ({
  ...
  return (
    <div className="pointer-events-none absolute inset-x-0 top-[8%] z-30 flex justify-center font-mono">
      <style>{HUD_KEYFRAMES}</style>
      {/* Outer shell — carries the tone-accent edge + ambient glow */}
      <div
        data-hud-anim
        className={`${BEZEL_OUTER} min-w-[280px] max-w-md p-[3px]`}
        style={{
          borderColor: `${accent}77`,
          boxShadow: `0 18px 50px -12px rgba(0,0,0,0.75), 0 0 30px ${accent}22, inset 0 1px 0 rgba(255,255,255,0.05)`,
          animation: `hudStripIn 0.5s ${EASE.spring} both`,
        }}
      >
        {/* Inner core */}
        <div className={`${BEZEL_INNER} px-4 py-2.5`}>
