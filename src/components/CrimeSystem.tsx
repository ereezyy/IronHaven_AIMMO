...
const CrimeSystem: React.FC<CrimeSystemProps> = ({
  playerPosition,
  nearbyNPCs,
  onCrimeCommitted,
}) => {
  ...
  useEffect(() => {
    const now = Date.now();
    const recentCrimeEvents = recentCrimes.filter(
      (crime) => now - crime.timestamp < 300000
    ); // 5 minutes
  ...
  useEffect(() => {
    ...
    if (crimeType) {
      ...
      const newWanted = Math.min(
        gameStore.playerStats.wanted + wantedIncrease,
        5
      );
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const now = Date.now();
      setRecentCrimes((prev) =>
        prev.filter((crime) => now - crime.timestamp < 600000)
      ); // Keep for 10 minutes
  ...
  return (
    <div className="absolute bottom-20 left-4 p-3 bg-black/90 text-white rounded-lg border border-red-500/70 backdrop-blur-sm">
      <h3 className="text-lg font-bold text-red-400 mb-2 border-b border-red-500/30 pb-1">
        CRIME STATUS
      </h3>
